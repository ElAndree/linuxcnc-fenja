# xhc-whb04.tcl: HALFILE for xhc-whb04 pendant
...
#   step = xhc-whb04.stepsize-up    (for usual behavior)
#   (see ini files for more exanples)

# Notes:
#    1) the 'start-pause' pin can be set to "std_start_pause" to
#       implement default behavior
#    2) the 'step' pin is normally connected to xhc-whb04.stepsize-up
...
proc connect_pins {} {
  foreach bname [lsort [array names ::XHC_WHB04_BUTTONS]] {
    set thepin $::XHC_WHB04_BUTTONS($bname)
...
proc wheel_setup {jogmode} {
  for {set idx 0} {$idx < 4} {incr idx} {
    set ::XHC_WHB04_CONFIG(accel,$idx) 1.0 ;# default if unspecified
  }
  if [info exists ::XHC_WHB04_CONFIG(mpg_accels)] {
    set idx 0
    foreach g $::XHC_WHB04_CONFIG(mpg_accels) {
      set g1 $g
      if {$g < 0} {
         set g [expr -1 * $g]
         puts stderr "$::progname: mpg_accel #$idx must be positive was:$g1, is:$g"
      }
      set ::XHC_WHB04_CONFIG(accel,$idx) [format %f $g] ;# ensure floatingpt
      incr idx
    }
  }

  # defaults if not in inifile:
  set ::XHC_WHB04_CONFIG(coef,0) 1.0
  set ::XHC_WHB04_CONFIG(coef,1) 1.0
  set ::XHC_WHB04_CONFIG(coef,2) 1.0
  set ::XHC_WHB04_CONFIG(coef,3) 1.0
  if [info exists ::XHC_WHB04_CONFIG(coefs)] {
    set idx 0
    foreach g $::XHC_WHB04_CONFIG(coefs) {
      set g1 $g
      if {$g < 0} {
         set g [expr -1 * $g]
      ...
      set ::XHC_WHB04_CONFIG(coef,$idx) $g
      incr idx
    }
  }
  # defaults if not in inifile:
  set ::XHC_WHB04_CONFIG(scale,0) 1.0
  set ::XHC_WHB04_CONFIG(scale,1) 1.0
  set ::XHC_WHB04_CONFIG(scale,2) 1.0
  set ::XHC_WHB04_CONFIG(scale,3) 1.0
  if [info exists ::XHC_WHB04_CONFIG(scales)] {
    set idx 0
    foreach g $::XHC_WHB04_CONFIG(scales) {
      set ::XHC_WHB04_CONFIG(scale,$idx) $g
      incr idx
    }
  }

  # to support fractional scales:
  #   divide the xhc-WHB04.jog.scale by $kvalue
  #   and
  ...
  # and xhc-whb04.cc hardcodes pin names for these letters: x y z a
  ...
  foreach coord $::XHC_WHB04_CONFIG(coords) {
  ...
  foreach coord $::XHC_WHB04_CONFIG(coords) {
    ...
    setp pendant_util.coef$idx  $::XHC_WHB04_CONFIG(coef,$idx)
    setp pendant_util.scale$idx [expr $kvalue * $::XHC_WHB04_CONFIG(scale,$idx)]
    ...
    if [catch {
      set afraction [expr  $::XHC_WHB04_CONFIG(accel,$idx)\
    ...
    if $has_jnum {
      if ![pin_exists joint.$jnum.jog-scale] {
        err_exit "Not configured for coords = $::XHC_WHB04_CONFIG(coords),\
      ...
      if [catch {
        set jfraction [expr  $::XHC_WHB04_CONFIG(accel,$idx)\
...
if {[array names ::XHC_WHB04_CONFIG] == ""} {
...
foreach name [array names ::XHC_WHB04_CONFIG] {
  set ::XHC_WHB04_CONFIG($name) [string trim $::XHC_WHB04_CONFIG($name) "{}"]
}
...
if ![info exists ::XHC_WHB04_CONFIG(require_pendant)] {
  set ::XHC_WHB04_CONFIG(require_pendant) yes ;# default
}
set dashx -x
switch $::XHC_WHB04_CONFIG(require_pendant) {
  no      {set dashx ""}
}

if [info exists ::XHC_WHB04_CONFIG(sequence)] {
  set dashs "-s $::XHC_WHB04_CONFIG(sequence)"
} else {
  set dashs ""
}
...
if [catch {eval $cmd} msg] {
  ...
  set msg "$msg Continuing without xhc-whb04\n"
  set msg "$msg \nFailing cmd:\n$cmd"
  popup_msg "$msg"
  return ;# not an exit
}

if ![info exists ::XHC_WHB04_CONFIG(inch_or_mm)] {
...
if ![info exists ::XHC_WHB04_CONFIG(jogmode)] {
  set ::XHC_WHB04_CONFIG(jogmode) normal ;# default
}

set jogmode $::XHC_WHB04_CONFIG(jogmode)
switch $jogmode {
  normal {}
  vnormal {}
  default {
    set ::XHC_WHB04_CONFIG(jogmode) normal
    set msg "Unkknown jogmode <$jogmode>"
    set msg "$msg  Using $::XHC_WHB04_CONFIG(jogmode)"
    popup_msg "$msg"
  }
}

if [info exists ::XHC_WHB04_CONFIG(coords)] {
  if ![is_uniq $::XHC_WHB04_CONFIG(coords)] {
    err_exit "coords must be unique, not: <$::XHC_WHB04_CONFIG(coords)>"
  }
  if {[llength $::XHC_WHB04_CONFIG(coords)] > 4} {
    err_exit "max no.of coords is 4 <$::XHC_WHB04_CONFIG(coords)>"
  }
} else {
  set ::XHC_WHB04_CONFIG(coords) {x y z a} ;# default
}
...
if { ![namespace exists ::tp] || ([::tp::passnumber] != 0) } {
  connect_pins    ;# per ini file items: [XHC_WHB04_BUTTONS]buttonname=pin
  wheel_setup  $::XHC_WHB04_CONFIG(jogmode)
                   # jog wheel per ini file items:
                   #     [XHC_WHB04_CONFIG]coords,coefs,scales
}
if [info exists ::makenet_msg] {
  popup_msg $::makenet_msg
}
#parray ::XHC_WHB04_CONFIG
